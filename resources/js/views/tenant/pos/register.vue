            ...
            <div class="col-6">
                <div class="card mb-0">
                    ...
                    <div class="card-footer bg-dark" style="height: 44vh">
                        <div class="row">
                            <div class="col-6">
                                ...
                                <div class="form-group">
                                    <button type="button" class="btn waves-effect waves-light btn-primary btn-xs" @click.prevent="showDialogAddItem = true">+ Agregar Producto Detallado
                                    </button>
                                </div>
                            </div>
                        ...
                        <div class="row pt-1">
                            <div class="col-3">
                                <el-button class="btn btn-danger btn-sm btn-block" type="danger" @click.prevent="close">
                                    Cancelar
                                </el-button>
                            </div>
                            <div class="col-9">
                                <el-button class="btn btn-submit btn-sm btn-block" type="primary" native-type="submit" @click.prevent="showDialogMakeSale=true" v-if="form.items.length > 0"> Procesar Pago
                                </el-button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-body">
                        ...
                        <div class="row mt-2">
                            ...
                            <div class="col-lg-12 col-md-6 d-flex align-items-end">
                                <div class="form-group">
                                    <button type="button" class="btn waves-effect waves-light btn-primary btn-xs" @click.prevent="showDialogAddItem = true">+ Agregar Producto
                                    </button>
                                </div>
                            </div>
                            <div class="col-md-12">
        ...
        <form autocomplete="off" @submit.prevent="submit">
            <el-dialog
                ...
                <div class="row" v-if="payment.total>0">
                    <div class="col-lg-4 col-md-4 col-sm-4">
                        <div class="form-group">
                            ...
                            <div>
                                <label v-for="option in document_types" :key="option.id" class="btn btn-primary btn-sm mr-1 mb-1" v-bind:for="'dt'+option.id"
                                    :class="{'active':form.document_type_id==option.id}">
                                    <input type="radio" @change="changeDocumentType" v-model="form.document_type_id" v-bind:value="option.id" v-bind:id="'dt'+option.id"/>
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-4">
                        <div class="form-group">
                            ...
                            <div>
                                <label
                                    v-for="option in series" :key="option" class="btn btn-info btn-sm mr-1 mb-1" v-bind:for="'idS'+option.id" :class="{'active':form.series_id==option.id}">
                                    <input type="radio" v-model="form.series_id" v-bind:value="option.id" v-bind:id="'idS'+option.id"/>
                                    {{option.number}}
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-4">
                        <div class="form-group">
                            <label class="control-label">Cliente
                                <a href="#" class="btn btn-xs btn-primary" @click.prevent="showDialogNewPerson = true">+</a>
                            </label>
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12">
                        <div class="form-group">
                            <label class="control-label">Forma de Pago
                                <button class="btn btn-success btn-xs" v-if="informacion_adicional.pagos.length<4" @click.prevent="informacion_adicional.pagos.push({tipo:''})">
                                    <i class="fas fa-plus"></i>
                                </button>
                            </label>
                            <div class="row" v-for="(info,index) in informacion_adicional.pagos" :key="info">
                                <div class="col-1 pt-4">
                                    <button class="btn btn-danger btn-xs" v-if="index>0" @click.prevent="informacion_adicional.pagos.splice(index,1)">
                                ...
                                <div class="col-3">
                                    <div class="form-group">
                                        ...
                                        <select v-model="info.tipo" class="form-control form-control-sm" required>
                                            <option v-for="option in payment_methods" :value="option.id" :key="option.id" >{{ option.description}}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-3">
                                    <div class="form-group">
                                        ...
                                        <select v-model="info.cuenta" class="form-control form-control-sm" required>
                                            <option v-for="option in accounts" :value="option.id" :key="option.id" >{{ option.name}}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-4">
                                    <div class="form-group">
                                        ...
                                        <input v-model="info.monto" class="form-control form-control-sm text-right" type="number" step="any" required>
                                    </div>
                                </div>
                                <!-- <div class="col-4">
                                    <div class="form-group" v-if="info.tipo!=='Efectivo' && info.tipo!==''">
                                        <label class="control-label">Num. Referencia</label>
                                        <input v-model="info.ref" class="form-control form-control-sm text-right" type="text" required>
                                    </div>
                                </div> -->
                            </div>
                        </div>
                ...
                <span slot="footer" class="dialog-footer">
                    <el-button size="mini" type="danger" @click.prevent="close" v-if="payment.total>0" class="btn-danger btn-xs">Cancelar venta</el-button>
                    <el-button type="warning" @click.prevent="showDialogMakeSale = false" class="btn-warning btn-lg">Atras</el-button>
                    <el-button type="primary" native-type="submit" v-if="payment.total>0" :loading="loading_submit" :disabled="payment.delta>0"
...
<script>
    ...
    export default {
        props: ['pos_station', 'pos_shift_id'],
        components: {
        ...
        async created() {
            ...
            await this.$http.get(`/${this.resource}/tables`)
                .then(response => {
                    ...
                    this.form.establishment_id = this.pos_station.establishment_id
                    this.form.document_type_id = (this.document_types.length > 0) ? this.document_types[0].id : null
                    this.form.operation_type_id = (this.operation_types.length > 0) ? this.operation_types[0].id : null

                    this.form.warehouse_id = this.pos_station.warehouse_id
            ...
            submit() {
                ...
                this.$http.post(`/${this.model}`, this.form).then(response => {
                    if (response.data.success) {
                        ...
                        var table_id_ = this.pos_station.id
                        var table_name_ = 'pos_stations'

                        if(this.pos_shift_id){
                            table_id_ = this.pos_shift_id
                            table_name_ = 'pos_shifts';
                        }
