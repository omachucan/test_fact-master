...
class DocumentController extends Controller
{
    ...
    {
        $payments = Payment::where('table_id', $document->id)->where('table_name', 'documents')->get();
    ...
    {
        $records = Document::with('pos')->where($request->column, 'like', "%{$request->value}%")
    ...
    {
        $document = Document::with('payments')->find($documentId);
    ...
    public function tables()
    {
        ...
        $exchange_rate = Configuration::first()->exchange_rate;
        ...
        return compact('customers', 'establishments', 'warehouse_id', 'warehouses', 'series', 'document_types_invoice', 'document_types_invoice2', 'document_types_note',
            ...
            'discount_types', 'charge_types', 'payment_methods', 'accounts', 'company', 'document_types_guide', 'document_type_03_filter', 'decimal','exchange_rate' ,'price_list');
    }
    ...
    public function tables2($quotation_id = false)
    {
        ...
        return compact('quotation', 'customers', 'establishments', 'warehouse_id', 'warehouses', 'series', 'document_types_invoice', 'document_types_note',
            'note_credit_types', 'note_debit_types', 'currency_types', 'operation_types',
            'discount_types', 'charge_types', 'company', 'document_types_guide', 'document_type_03_filter','exchange_rate' ,'decimal');
    }
    ...
    public function store(DocumentRequest $request)
    {
        ...
        $this->store_sunat_information($document->id, $response);
    ...
    public function send($document_id)
    {
        ...
        $fact = DB::connection('tenant')->transaction(function () use ($document) {
            ...
            $DevPro->loadXmlSigned();
            $DevPro->onlySenderXmlSignedBill();
    ...
    public function delete($documet_id)
    {
        $register = DB::connection('tenant')->transaction(function () use ($documet_id)
        {
            $document = Document::find($documet_id);
            $documentItems = DocumentItem::where('document_id', $documet_id)->get();


            \App\Models\Tenant\InventoryKardex::where('inventory_kardexable_id', $documet_id)
                                                ->where('inventory_kardexable_type', 'App\Models\Tenant\Document')
                                                ->delete();

            foreach($documentItems as $documentItem)
            {
                $item_warehouse = \App\Models\Tenant\ItemWarehouse::where('warehouse_id', $document->warehouse_id)->where('item_id', $documentItem->item_id)->first();
            ...
            WHERE pay.table_id = $documet_id AND pay.table_name = 'documents'");

            \App\Models\Tenant\Payment::where('table_id', $documet_id)->where('table_name', 'documents')->delete();

            DocumentItem::where('document_id', $documet_id)->delete();
            Document::where('id', $documet_id)->delete();
        });

        return $register;
    }

    public function store_sunat_information($document_id, $response)
    {
        $document_sunat = Document::find($document_id);
