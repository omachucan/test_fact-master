            ...
            <div class="card-body ">
                <data-table :resource="resource">
                    ...
                    <tr slot-scope="{ index, row }" slot="tbody">
                        ...
                        <td class="text-right">
                            <span class="badge bg-secondary text-white" :class="{'bg-danger': (row.state_type_id === '11')}">{{ row.state_type_description }}</span>
                        </td>
                        <td class="text-right">
                            <el-tooltip class="item" effect="dark" content="Agregar pago" placement="top-end">
                                <button type="button" class="btn btn-xs" @click.prevent="clickPay(row.id)" v-if="row.total_to_pay > 0 && row.has_pay"><i class="fa fa-money-bill-wave i-icon text-warning"></i></button>
                            ...
                            <el-tooltip v-show="hasPermissionTo('tenant.sale-notes.edit')" v-if="row.has_edit" class="item" effect="dark" content="Editar Nota Venta" placement="top-end">
                                <a :href="`/sale-notes/edit/`+row.id" class="btn btn-xs"><i class="fa fa-file-signature i-icon text-success"></i></a>
                            </el-tooltip>
                            <el-tooltip v-show="hasPermissionTo('tenant.sale-notes.destroy')" class="item" effect="dark" content="Anular" placement="top-end">
                                <button type="button" class="btn btn-xs" @click.prevent="clickCancel(row.id)" v-if="row.has_delete"><i class="fa fa-ban i-icon text-danger"></i></button>
                                <button type="button" class="btn btn-xs" v-else><i class="fa fa-ban i-icon text-disabled"></i></button>
                            </el-tooltip>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            clickCancel(id) {
                this.cancel(`/${this.resource}/cancel/${id}`).then(() =>
