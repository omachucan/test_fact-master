...
class PersonController extends Controller
{
    ...
    public function view($type, Person $person)
    {
        ...
        $sql = "SELECT tab.id, tab.`date_of_issue`, tab.series, tab.number, tab.`currency_type_id`, tab.total, 'Venta' AS operation_type, NULL AS detail
                FROM payments pay
                INNER JOIN documents tab ON tab.id = pay.table_id
                WHERE tab.`customer_id` = $person->id AND pay.table_name = 'documents'
                UNION ALL
                ...
                FROM payments pay
                INNER JOIN sale_notes tab ON tab.id = pay.table_id
                WHERE tab.`customer_id` = $person->id AND pay.table_name = 'sale_notes'
